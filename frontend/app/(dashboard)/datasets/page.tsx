  ...
  return (
    <div className="flex gap-1">
      <button onClick={() => handleDownload("csv")} disabled={downloading !== null}
        className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 bg-surface-variant text-on-surface">
  ...
  return (
    <div className="space-y-lg">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-headline-lg font-bold tracking-tight text-on-surface">Data Sources</h2>
          <p className="text-body-md mt-1 text-on-surface-variant">Centralize all your metrics by connecting data sources.</p>
        </div>
        <Link href="/datasets/upload"
          className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all active:scale-95 bg-primary text-on-primary">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search data sources..."
            className="w-full rounded-lg pl-9 pr-4 py-2 text-sm bg-surface-container-high text-on-surface border-none" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-6 bg-surface-container">
            <span className="material-symbols-outlined text-[40px] text-on-surface-variant">database</span>
          </div>
          <h3 className="font-headline-md mb-2 text-on-surface">No data sources yet</h3>
          <p className="text-sm mb-6 text-on-surface-variant">Upload a CSV or Excel file to get started.</p>
          <Link href="/datasets/upload" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium transition-all active:scale-95 shadow-lg bg-primary text-on-primary">
            <Plus className="w-5 h-5" /> Upload your first dataset
          </Link>
          {/* Custom integration CTA */}
          <div className="mt-12 px-lg py-lg rounded-xl border-2 border-dashed border-outline-variant">
            <span className="material-symbols-outlined text-[36px] text-on-surface-variant">hub</span>
            <h4 className="font-headline-md mt-2 text-on-surface">Need a custom integration?</h4>
            <p className="text-sm mt-1 mb-4 text-on-surface-variant">Our engineering team can build custom connectors for your proprietary APIs.</p>
            <button className="px-lg py-2 rounded-full font-label-sm uppercase tracking-widest text-sm transition-all active:scale-95 border border-primary text-primary">
              Request Integration
            </button>
          </div>
        </div>
              ...
              <div key={dataset.id} className="glass-card rounded-xl p-lg flex flex-col justify-between transition-all hover:translate-y-[-2px]">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="h-12 w-12 rounded-xl flex items-center justify-center bg-surface-container">
                  ...
                  <h3 className="font-headline-md mb-1 text-on-surface">{dataset.name}</h3>
                  {dataset.description && <p className="text-sm mb-3 line-clamp-2 text-on-surface-variant">{dataset.description}</p>}
                  <div className="flex items-center gap-3 text-xs mb-4 text-on-surface-variant">
                  ...
                  <p className="text-xs text-on-surface-variant">Synced {formatDate(dataset.created_at)}</p>
                </div>
                <div className="flex gap-2 mt-5 pt-4 border-t border-outline-variant">
                  {dataset.status === "ready" && (
                    <>
                      <button onClick={() => handleGenerateDashboard(dataset.id, dataset.name)} disabled={generating === dataset.id}
                        className="flex-1 py-2 rounded-lg text-xs font-medium transition-all active:scale-95 disabled:opacity-50 bg-primary text-on-primary">
                  ...
                  {dataset.status !== "ready" && (
                    <span className="flex-1 py-2 rounded-lg text-xs text-center font-medium bg-surface-container-high text-on-surface-variant">
                      Processing...
                    </span>
                  )}
                  <button onClick={() => handleDelete(dataset.id)} className="p-2 rounded-lg transition-colors text-on-surface-variant">
