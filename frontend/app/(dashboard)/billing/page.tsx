...
function BillingContent() {
  ...
  return (
    <div className="max-w-5xl space-y-lg">
      <div>
        <h1 className="text-headline-lg font-bold text-on-surface">Billing & Plans</h1>
        <p className="text-sm text-on-surface-variant">Currently on the <span className="font-medium capitalize text-primary">{currentPlan}</span> plan</p>
      </div>

      {success && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-tertiary-container text-on-tertiary-container">
          <CheckCircle className="w-5 h-5 shrink-0" /><p className="font-medium">Subscription activated! Your plan has been upgraded.</p>
        </div>
      )}
      {canceled && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-surface-container text-on-surface">
            ...
            <div key={plan.id} className="relative glass-card rounded-xl overflow-hidden flex flex-col">
              {plan.popular && (
                <div className="absolute top-3 right-3 z-10">
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-primary text-on-primary">Most Popular</span>
                </div>
              )}
              <div className="px-lg py-lg bg-surface-container">
                <h3 className="font-headline-md font-bold text-on-surface">{plan.name}</h3>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-headline-md font-bold text-on-surface">{plan.price}</span>
                  <span className="text-sm text-on-surface-variant">{plan.period}</span>
                </div>
                <p className="text-sm mt-1 text-on-surface-variant">{plan.description}</p>
              </div>
              <div className="p-lg flex-1 flex flex-col">
                <ul className="space-y-2.5 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm text-on-surface-variant">
                      <Check className="w-4 h-4 shrink-0 mt-0.5 text-tertiary" />{f}
                    </li>
                  ))}
                </ul>
      ...
      {currentPlan !== "free" && (
        <div className="flex items-center justify-between p-lg rounded-xl bg-surface-container-low border border-outline-variant">
          <div>
            <p className="font-medium text-on-surface">Manage Subscription</p>
            <p className="text-sm text-on-surface-variant">Update payment method, cancel, or change plan via Stripe portal</p>
          </div>
          <button onClick={handlePortal} disabled={portalLoading}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all active:scale-95 bg-surface-container-high text-on-surface">
